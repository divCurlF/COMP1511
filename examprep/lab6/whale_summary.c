...
void whale_summary(int n_sightings, struct pod sightings[n_sightings]) {
	struct tally tallies[MAX_WHALE_SPECIES];
	int num_tallies = 0;
	int found;
	...
	while (i < n_sightings) {
		...
		found = 0;
		while (j < num_tallies) {
			if (strcmp(tallies[j].species, sightings[i].species) == 0) {
				tallies[j].pods++;
				tallies[j].whales += sightings[i].how_many;
				found = 1;
				break;
			}
			j++;
		...
		if (!found) {
			tallies[num_tallies].pods = 1;
			tallies[num_tallies].whales = sightings[i].how_many;
			strcpy(tallies[num_tallies].species, sightings[i].species);
			num_tallies++;
		}
		i++;
	}
	...
	i = 0;
	while (i < num_tallies) {
		printf("%2d %s containing %d whales\n", tallies[i].pods, tallies[i].species, tallies[i].whales);
		i++;
	}
}
