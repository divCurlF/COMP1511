...
int sameDate(struct date *d1, struct date *d2);
...
void merge_day_whales(struct pod *first_pod) {
    ...
    while (curr->next != NULL) {
    	...
    	while (inner_curr->next != NULL) {
    		...
    		if (sameDate(curr->when, inner_curr->next->when)
    		&& (strcmp(curr->species, inner_curr->next->species) == 0)) {
    			temp = inner_curr->next->next;
    			
    			curr->how_many += inner_curr->next->how_many;
    			
    			free(inner_curr->next->species);
    			free(inner_curr->next->when);
    			free(inner_curr->next);
    			
    			inner_curr->next = temp;
...
// return 1 if d1 the same date as d2
int sameDate(struct date *d1, struct date *d2) {
