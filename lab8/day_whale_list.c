...
int sameDate(struct date *day, struct date *next);
...
void day_whales(struct pod *first_pod, struct date *day) {
	...
	while (curr->next != NULL) {
		if (sameDate(day, curr->next->when)) {
			printf("%d %s\n", curr->next->how_many, curr->next->species);
		}
		curr = curr->next;
	}
}

int sameDate(struct date *day, struct date *entry_day) {
	return (day->day == entry_day->day &&
		day->month == entry_day->month &&
		day->year == entry_day->year
		);
}
