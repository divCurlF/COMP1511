...
int betweenDay(struct date *start_day, struct date *finish_day, struct date *entry_day);
...
void between_days_whales(struct pod *first_pod, struct date *start_day, struct date *finish_day) {
	...
	while (curr != NULL) {
		if (betweenDay(start_day, finish_day, curr->when)) {
...
int betweenDay(struct date *start_day, struct date *finish_day, struct date *entry_day) {
	return (
		(compareDay(start_day, entry_day) >= 0) &&
		(compareDay(entry_day, finish_day) >= 0)
	       );
}
// PUT YOUR FUNCTIONS HERE

// If two dates are the same return 0, if d1 < d2 return 1 if d2 > d1 return -1;
int compareDay(struct date *d1, struct date *d2) {
	if (d1->year < d2->year) {
		return 1;
